...
\node at (1.5,3.4){Panel I};

\def\s{0.45}

% -------- black square PQRS remaining part --------

% Row 5 (top)
\foreach \x in {0,1,2,3,4,5}{
    \fill (\x*\s,5*\s) rectangle ++(\s,\s);
}

% Row 4
\foreach \x in {0,3}{
    \fill (\x*\s,4*\s) rectangle ++(\s,\s);
}

% Row 3
\foreach \x in {0,1,2}{
    \fill (\x*\s,3*\s) rectangle ++(\s,\s);
}

% Row 2
\foreach \x in {0,4,5}{
    \fill (\x*\s,2*\s) rectangle ++(\s,\s);
}

% Row 1
\foreach \x in {0,4,5}{
    \fill (\x*\s,1*\s) rectangle ++(\s,\s);
}

% Row 0 (bottom)
\foreach \x in {0,1,2,3,4,5}{
    \fill (\x*\s,0) rectangle ++(\s,\s);
}


% labels
\node at (-0.12,6*\s+0.1){S};
\node at (6*\s+0.12,6*\s+0.1){R};

\node at (-0.12,-0.15){P};
\node at (6*\s+0.12,-0.15){Q};
...
% dotted lines
\draw[dashed](-0.4,0.45)--(5.5,0.45);
\draw[dashed](2.7,-1.8)--(2.7,3.4);
...
\newcommand{\tile}[3]{
\draw (#1,#2) rectangle ++(1.2,1.2);

\foreach \x in {0.3,0.6,0.9}{
\foreach \y in {0.3,0.6,0.9}{
\draw (#1+\x,#2+\y) circle (0.055);
}}
...
\draw[dashed] (2.4,1.8) rectangle ++(1.2,1.2);
\node at (3,2.4){\Large ?};
...
% solid divider
\draw[line width=0.8pt] (4.8,0)--(4.8,5);
...
\node at (2.5,4.8){Panel II};


% helper stacked option
\newcommand{\optionbox}[3]{
...
% separators
\draw[dashed] (-0.5,1.25)--(5.5,1.25);
\draw[dashed] (2.5,-2.1)--(2.5,4.4);
...
% option iv
\begin{scope}[xshift=3.5cm,yshift=-3.4cm]
...
Based on the figures in Panel I and Panel II, select the appropriate choice for the missing tiles.
}
\\
\hline
...
A salesperson needs to make a trip. She needs to start from a city, visit each of
the remaining cities exactly once, and finally return to the same city from which
she started.
...
\begin{tikzpicture}[
scale=0.75,
city/.style={circle, fill=black, inner sep=2.5pt}
]

%---------------- Figure (i) ----------------%

\node at (1.5,3.5) {\textbf{(i)}};
...
\node at (1.4,3.5) {\textbf{(ii)}};

\node[city] (A) at (0,3){};
\node[city] (B) at (3,3){};
\node[city] (C) at (1.7,2.2){};
\node[city] (D) at (1.4,0.4){};
\node[city] (E) at (-0.4,1.1){};
...
% outer box
\draw (#1,#2) rectangle (#1+2,#2+2);

% vertical lines
\draw (#1+0.5,#2)--(#1+0.5,#2+2);
\draw (#1+1,#2)--(#1+1,#2+2);
\draw (#1+1.5,#2)--(#1+1.5,#2+2);

% horizontal lines
\draw (#1,#2+0.5)--(#1+2,#2+0.5);
\draw (#1,#2+1)--(#1+2,#2+1);
\draw (#1,#2+1.5)--(#1+2,#2+1.5);

}

\newcommand{\gcell}[2]{
\fill[gray!45] (#1,#2) rectangle ++(0.5,0.5);
}


% ================= PANEL I =================

\node at (1,3.4){Panel I};
...
\draw[line width=1pt] (2.8,-1.6)--(2.8,3.5);
...
\draw[dashed] (-0.4,0.45)--(5.4,0.45);

\draw[dashed] (2.5,-2.5)--(2.5,3.1);
...
Q.36 &
\parbox[t]{\linewidth}{
Consider a complete graph $K_n$ with $n$ vertices $(n>4)$. Note that
multiple spanning trees can be constructed over $K_n$. Each of these
spanning trees is represented as a set of edges.

The Jaccard coefficient between any two sets is defined as the ratio
of the size of the intersection of the two sets to the size of the union
of the two sets.

Which one of the following options gives the lowest possible value
for the Jaccard coefficient between any two spanning trees of $K_n$?
...
(A) & 
\raisebox{-0.15cm}{$\displaystyle\frac{1}{n}$}
\\[0.35cm]
\hline

(B) &
\raisebox{-0.15cm}{$\displaystyle\frac{1}{2n-3}$}
\\[0.35cm]
\hline
...
(D) &
\raisebox{-0.15cm}{$\displaystyle\frac{1}{n-1}$}
\\[0.35cm]
\hline
...
\parbox[t]{\linewidth}{
\strut
Let $G$ be a weighted directed acyclic graph with $m$ edges and $n$
vertices. Given $G$ and a source vertex $s$ in $G$, which one of the
following options gives the worst case time complexity of the fastest
algorithm to find the lengths of shortest paths from $s$ to all vertices
...
\parbox[t]{\linewidth}{
\strut
Consider an array $A$ of integers of size $n$. The indices of $A$ run from
$1$ to $n$. An algorithm is to be designed to check whether $A$ satisfies
the condition given below.

\[
\forall i,j \in \{1,\ldots,n-1\}\ \text{such that } i>j,
\]
\[
(A[i+1]-A[i]) > (A[j+1]-A[j])
\]

Which one of the following gives the worst case time complexity of
the fastest algorithm that can be designed for the problem?
...
(B) & $\Theta(\log(n))$ \\[0.25cm]
\hline

(C) & $\Theta(n\log(n))$ \\[0.25cm]
\hline
...
\small
\vspace{0.05cm}
Consider a table $T$, where the elements $T[i][j]$, $0\leq i,j\leq n$,
represent the cost of the optimal solutions of different subproblems
of a problem that is being solved using a dynamic programming algorithm.

The recursive formulation to compute the table entries is as follows:

\[
T[0][k]=T[k][0]=1 \qquad \text{for } k=0,1,2,\ldots,n
\]

\[
T[i][j]=2T[i-1][j]+3T[i][j-1]\qquad \text{for }1\leq i,j\leq n
\]

Consider the following two algorithms to compute entries of $T$.
Assume that for both algorithms, for all $0\leq i,j\leq n$, $T[i][j]$
...
\[
\begin{array}{c}
\text{For } i=1,2,\ldots,n\\[0.25cm]
\text{For } j=1,2,\ldots,n\\[0.25cm]
...
Algorithm $B_k$, $k\in\{1,2\}$ said to be correct if and only if it
calculates the correct values of $T[i][j]$ for all $0\leq i,j\leq n$,
...
(A) & Both algorithms $B_1$ and $B_2$ are correct \\[0.25cm]
\hline

(B) & Algorithm $B_1$ is correct, but algorithm $B_2$ is incorrect \\[0.25cm]
\hline

(C) & Algorithm $B_2$ is correct, but algorithm $B_1$ is incorrect \\[0.25cm]
\hline

(D) & Both algorithms $B_1$ and $B_2$ are incorrect \\[0.25cm]
\hline
...
\textit{Note:} $+$ is OR operation, $\cdot$ is AND operation,
$'$ is NOT operation
...
\parbox[t]{\linewidth}{
\strut
Consider the canonical $LR(0)$ parsing of the grammar below using
terminals $\{a,b,c\}$ and non-terminals $\{A,B,C,S\}$ with $S$ as the
start symbol.
...
Which one of the following options gives the number of shift-reduce
conflicts that will occur in the $LR(0)$ ACTION table?
...
\parbox[t]{\linewidth}{
\strut
In the context of schema normalization in relational DBMS, consider a set
$F$ of functional dependencies. The set of all functional dependencies
implied by $F$ is called the closure of $F$.
...
It can be proved that the additional rule of Union is also implied
by the three rules of Armstrong's Axioms.

Listed below are four combinations of these three rules. Which one of
these combinations is both necessary and sufficient for the proof?
...
(A) & Reflexivity, Augmentation, and Transitivity \\[0.25cm]
\hline

(B) & Reflexivity and Augmentation \\[0.25cm]
\hline

(C) & Transitivity \\[0.25cm]
\hline

(D) & Augmentation and Transitivity \\[0.25cm]
\hline
...
\parbox[t]{\linewidth}{
\strut
Consider the transmission of data bits 11001011 over a link that uses
Cyclic Redundancy Check (CRC) code for error detection.

If the generator bit pattern is given to be 1001, which one of the
following options shows the remainder bit pattern appended to the data
...
Consider a processor that has 16 general purpose registers and it uses 
2-byte instruction format for all its instructions. Variable-sized opcodes 
are permitted. There are three different types of instructions; M-type, 
R-type, and C-type.

Each M-type instruction has 2 register operands and a 6-bit immediate 
operand. Each R-type instruction has 3 register operands. Each C-type 
instruction has a register operand and a 6-bit offset value.

If there are 2 unique M-type opcodes and 7 unique R-type opcodes, which 
one of the following options gives the maximum number of unique opcodes 
possible for C-type instructions?
...
\node (entry) at (0,2.8) {ENTRY};

\node[block] (B1) at (0,1.8) {$a=b+c$};
\node[left] at (-1.25,2.15) {B1};

\node[block] (B2) at (0,0.5) {$d=a+e$};
\node[left] at (-1.25,0.8) {B2};

\node[block] (B3) at (2.7,0.5) {$e=a+f$};
\node[above] at (2.7,0.95) {B3};

\node[block] (B4) at (0,-0.8) {$g=d+e$};
\node[left] at (-1.25,-0.5) {B4};

\node (exit) at (0,-2) {EXIT};
...
% B1 to B3 (direct diagonal)
\draw[->] (B1.south east) -- (B3.north west);
...
Which one of the following options is the set of live variables at the
exit point of each basic block?
...
(A) & B1:\{a,b,c,f\}, B2:\{d,e\}, B3:\{b,c,e,f\}, B4:$\emptyset$ \\[0.25cm]
\hline

(B) & B1:$\emptyset$, B2:\{d,e\}, B3:\{a,c,f\}, B4:$\emptyset$ \\[0.25cm]
\hline

(C) & B1:\{a,b,c,e,f\}, B2:\{d,e\}, B3:\{c,e,f\}, B4:$\emptyset$ \\[0.25cm]
\hline

(D) & B1:$\emptyset$, B2:\{d,e,f\}, B3:\{a,b,c,e,f\}, B4:$\emptyset$ \\[0.25cm]
\hline
...
\parbox[t]{\linewidth}{
\strut
An index in a DBMS is said to be dense if an index entry appears
for every search-key value in the indexed file. Otherwise it is called
a sparse index.
...
(A) & Both S1 and S2 are true \\[0.25cm]
\hline

(B) & Both S1 and S2 are false \\[0.25cm]
\hline

(C) & S1 is true and S2 is false \\[0.25cm]
\hline

(D) & S1 is false and S2 is true \\[0.25cm]
\hline
...
scale=0.8,
transform shape,
state/.style={circle,draw,minimum size=0.38cm},
accept/.style={circle,draw,double,minimum size=0.38cm},
>=stealth
]
...
\node[accept] (A) at (0,0) {};
\node[state] (B) at (1.4,0.8) {};
\node[state] (C) at (2.8,1.1) {};
\node[state] (D) at (4,1.5) {};
\node[state] (E) at (1.6,-1.2) {};
\node[state] (F) at (-0.1,-2) {};
...
% States
\node[accept] (A2) at (0,0) {};

\node[state] (B2) at (1.45,0.35) {};
\node[state] (C2) at (3.0,0.35) {};

\node[state] (D2) at (1.45,-1.15) {};
\node[state] (E2) at (0,-1.8) {};
...
% Big upper curve
\draw[->]
(C2) to[out=140,in=40,looseness=1.6]
node[above]{1} (A2);
...
% Bottom connections
\draw[->]
(D2) -- node[above left]{0} (E2);

\draw[->]
(E2) -- node[left]{0} (A2);


% OUTSIDE bottom curve (fixed)
\draw[->]
(D2) to[out=-100,in=-20,looseness=1.4]
node[below]{1} (E2);
...
Which of the following constraints ensure(s) that the language $L$ is
context-free?
...
Consider contiguous allocation of physical memory to processes using
variable partitioning scheme. Suppose there are 8 holes in the memory
of sizes 20 KB, 4 KB, 25 KB, 18 KB, 7 KB, 9 KB, 15 KB,
and 12 KB. Assume that no two holes are adjacent. Two processes
P1 of size 16 KB and P2 of size 9 KB arrive in that order, and they
are allocated memory using the best-fit technique. After allocating
space to P1 and P2, the number of holes of size less than 8 KB is
...
Consider a system with 1 MB physical memory and a word length
of 1 byte. The system uses a direct mapped cache, with block
numbers starting from 0. The word with physical address 0xA2C28 is
mapped to the cache block number $176_{10}$. The maximum possible
size of the cache (in KB) for this configuration is
\underline{\hspace{2cm}}.
...
A non-pipelined instruction execution unit that operates at 1.6 GHz
clock takes an average of 5 clock cycles to complete the execution
of an instruction. To improve the performance, the system was
pipelined with a goal of achieving an average throughput of one
instruction per clock cycle. However, it could operate only at
1.2 GHz due to pipeline overheads. While executing a program in the
pipelined design, 30\% of instructions encountered a stall of 2 cycles
due to pipeline hazards. The speed-up obtained by the pipelined
design over the non-pipelined one for this program is
\underline{\hspace{2cm}}.

(\textit{rounded off to two decimal places})
...
The receiver advertised window is constant at 48 KB, the maximum
segment size (MSS) is 2 KB, and the slow start threshold for TCP
congestion control is 16 KB. Assume that there are no timeouts
or duplicate acknowledgements. The number of rounds of transmission
required for the congestion control algorithm of the TCP connection
to reach the congestion avoidance phase is
\underline{\hspace{2cm}}.
...
% ---------------- Decoder block Q ----------------
\draw (0,0) rectangle (1.6,2.4);

\node at (0.8,1.2) {\Large Q};

\node at (1.35,2.0) {\scriptsize D0};
\node at (1.35,1.4) {\scriptsize D1};
\node at (1.35,0.8) {\scriptsize D2};
\node at (1.35,0.25) {\scriptsize D3};

% inputs A B
\draw[->] (-1.2,1.8)--(0,1.8);
\node[left] at (-1.2,1.8) {A};

\draw[->] (-1.2,0.6)--(0,0.6);
\node[left] at (-1.2,0.6) {B};


% ---------------- MUX block M ----------------
\draw (3.6,0) rectangle (5.2,2.4);

\node at (4.4,1.2) {\Large M};
...
% D0 to mux 0
\draw[->] (1.6,2.0)--(3.6,2.0);

% constant 0 to mux 1
\draw[->] (2.4,1.5)--(3.6,1.5);
\node[left] at (2.4,1.5) {0};

% constant 1 to mux 2
\draw[->] (2.4,0.9)--(3.6,0.9);
\node[left] at (2.4,0.9) {1};

% D3 to mux 3
\draw[->] (1.6,0.25)--(3.6,0.25);


% select inputs
\draw[->] (3.9,3)--(3.9,2.4);
\node[above] at (3.9,3) {S1};

\draw[->] (4.8,3)--(4.8,2.4);
\node[above] at (4.8,3) {S0};


% output Y
\draw[->] (5.2,1.2)--(6.3,1.2);
\node[right] at (6.3,1.2) {Y};
